...
/*
original code from:
https://github.com/e-tinkers/esp32-adc-calibrate/tree/master


taken from the readme.md:
    It is your choice on whether you want to implement the polynomial fitting method
    or LUT method to address your ESP32 ADC linearity problem.
    The LUT take up a lot of memory (which is not a big issue for ESP32) but it is
    faster and more accurate than the polynomial fitting approach.

How to generate the LUT?
    This program use the ESP32 DAC to generate a value as the reference and feed it
    into the ADC for calibration. Remember to use a short jumper wire to connect
    DAC output(GPIO 25) to ADC Channel 1 ADC7(GPIO 35) on your ESP32.

    By default, the program will generate a float ADC_LUT which take up much more memory
    but with better precision, if you want to have an int ADC_LUT table,
    comment out the line #define FLOAT_LUT to get the integer table.

    Connect a jumper wire between GPIO 25 and GPIO 35 (i.e. A7);
    decide on what table (float or integer) you'd want to generate;
    Start the main.cpp sketch from PlatformIO or Arduino IDE;
    When the program stops, copy and paste the entire table from Serial Monitor to your sketch to use it.

How to use the LUT?
    Refer to the sketch in example directory on how to apply the LUT in your program.
    Run the sketch and see the result on Serial Plotter.
